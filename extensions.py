...
# ── Rate limiter ──────────────────────────────────────────────────────────────
# Redis storage when REDIS_URL is configured (for 5,000+ concurrent users across
# multiple app instances/workers), falling back to in-memory storage for single-node dev.
_storage_uri = os.environ.get("REDIS_URL", "memory://").strip()
limiter = Limiter(
    get_remote_address,
    app=app,
