"""Employee portal blueprint — self-service portal, profile, ID card, mobile auth."""
import base64
import calendar
import datetime
import io
...
from utils.auth import (admin_required, employee_required,
                        ...
                        _hash_token, _check_login_lockout,
                        _record_login_failure, _clear_login_failures)
from utils.helpers import (_audit, get_company_settings, get_auth_config,
                           encrypt_pii, decrypt_pii,
                           _validate_image_file, _db)
from utils.attendance_utils import (
    _td_to_time, classify_by_worked_minutes, infer_type_legacy,
...
@employee_portal_bp.route("/api/employee/login", methods=["POST"])
...
@employee_portal_bp.route("/api/employee/logout", methods=["POST"])
...
@employee_portal_bp.route("/api/employee/mobile-biometric-nonce", methods=["POST"])
...
@employee_portal_bp.route("/api/employee/mobile-biometric-attest", methods=["POST"])
@limiter.limit("10 per minute")
@employee_api_required
