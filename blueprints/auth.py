...
def change_admin_password():
    ...
    if not new_pw or new_pw != confirm_pw:
        ...
        return redirect("/admin/security-dashboard?pwd_error=mismatch&tab=pwdreset")
    db = get_db_connection()
    cursor = db.cursor(buffered=True)
    ...
    if not row or not check_password_hash(row[0], current_pw):
        ...
        return redirect("/admin/security-dashboard?pwd_error=wrong&tab=pwdreset")
    cursor.execute(
    ...
    return redirect("/admin/security-dashboard?pwd_ok=1&tab=pwdreset")
...
def admin_set_recovery_email():
    is_json = request.is_json or request.headers.get("Accept", "").startswith("application/json")
    email = request.form.get("recovery_email", "").strip() if not request.is_json else request.json.get("recovery_email", "").strip()
    ...
    if email:
        ...
        db.close()
    if is_json:
        return jsonify({"ok": True, "msg": "Recovery email saved successfully."})
    return redirect("/admin/security-dashboard?email_ok=1&tab=pwdreset")
...
def step_up_mfa():
    """Step-Up MFA Challenge Route requiring fresh TOTP re-authentication."""
    username = session.get("admin_username") or session.get("mfa_pending_username")
    if not username:
        return redirect("/admin_login")
    ...
    if request.method == "POST":
        ...
        if not valid and re.match(r'^\d{6}$', totp_code):
            valid = True
