...
def admin_action():
    ...
    if action in ("register", "holiday", "salary", "update_face") and session.get("admin_role") != "admin":
...
<div style="font-family:'Segoe UI',sans-serif;max-width:520px;margin:0 auto;background:#f8fafc;padding:32px 24px;border-radius:16px;">
  ...
  <p style="color:#64748b;font-size:12px;text-align:center;margin:0;">This is an automated message — please do not reply.</p>
        ...
        emp_id = request.form.get("emp_id", "").strip() if not request.is_json else request.json.get("emp_id", "").strip()
        cursor.execute("SELECT name FROM employees WHERE employee_id=%s", (emp_id,))
        row = cursor.fetchone()
        if not row:
            if is_json:
                cursor.close()
                db.close()
                return jsonify({"ok": False, "msg": f"Employee ID '{emp_id}' not found."}), 404
            flash(f"Employee ID '{emp_id}' not found.", "error")
        else:
            cursor.execute(
            ...
            if is_json:
                cursor.close()
                db.close()
                return jsonify({"ok": True, "msg": f"Password reset for '{row[0]}' ({emp_id}). They can now login using their Employee ID as the password."})
            flash(
                f"Password reset for '{row[0]}' ({emp_id}). They can now login using their Employee ID as the password.", "success")
        cursor.close()
        db.close()
        return redirect("/admin/security-dashboard?tab=pwdreset")
