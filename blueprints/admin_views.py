...
@limiter.limit("10 per minute")
def api_soc_lock():
    """Explicit re-lock, mirroring /api/settings/2fa/lock."""
    soc_step_up_clear()
    return jsonify({"ok": True})


def _compute_security_posture():
    """Real, config-derived facts."""
    return {
        "hsts_enabled": True,
        "csp_enabled": True,
...
def _security_events_summary(cursor):
    """Aggregate stats over the FULL security_events history."""
...
def soc_security_dashboard():
    ...
    # 1. Fetch compromised sessions
    compromised_sessions = []
    try:
        cur.execute(
            "SELECT sid, identifier, attempt_type, score, status, last_reason FROM session_risk WHERE status='compromised'"
        )
        for r in cur.fetchall():
            compromised_sessions.append({
                ...
                "score": r[3],
                "status": r[4],
                "last_reason": r[5] or "Unusual behavioral pattern detected"
            })
    except Exception as exc:
        app_log.warning("Could not fetch compromised sessions: %s", exc)

    # 2. Compute security posture summary
    posture = _compute_security_posture()

    # 3. Security events summary statistics
    summary = {}
    try:
        summary = _security_events_summary(cur)
    except Exception as exc:
        app_log.warning("Could not build security events summary: %s", exc)

    # 4. Fetch admin roster for MFA enrollment check
    admins = []
    try:
        cur.execute("SELECT username, role, totp_enabled FROM admin_users ORDER BY username")
        for r in cur.fetchall():
            admins.append({
                "username": r[0],
                "role": r[1],
        ...
        app_log.warning("Could not fetch admin roster: %s", exc)

    # 5. Fetch banned IPs
    banned_ips = []
    try:
        cur.execute("SELECT ip, reason, banned_at, expires_at FROM banned_ips ORDER BY banned_at DESC LIMIT 50")
        for idx, r in enumerate(cur.fetchall(), 1):
            banned_ips.append({
                "id": idx,
                "ip": r[0],
                "reason": r[1] or "Security threat",
                "banned_at": str(r[2]) if r[2] else "—",
                "expires_at": str(r[3]) if r[3] else "Permanent",
            })
    except Exception as exc:
        app_log.warning("Could not fetch banned IPs: %s", exc)

    cur.close()
    db.close()

    # Import utilities for template rendering
    from utils.security_logs import fetch_threat_logs, get_port_health_metrics, get_quarantined_files, get_system_health_metrics, get_smtp_config
    threat_logs = fetch_threat_logs(limit=40)
    port_health = get_port_health_metrics()
    quarantined_files = get_quarantined_files()
    health_metrics = get_system_health_metrics()
    smtp_cfg = get_smtp_config()

    return render_template(
        "soc_security_dashboard.html",
...
_ASSIGNABLE_ROLES = ("admin", "manager", "soc_analyst", "cybersecurity")
