...
def _create_pool(retries=5, delay=3):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            # Fallback to UNIX domain socket if TCP connection was refused and socket exists
            if config.get("host") not in ("/tmp", "localhost") and os.path.exists("/tmp/.s.PGSQL.5432"):
                _log.info('Falling back to UNIX domain socket /tmp for PostgreSQL')
                config["host"] = "/tmp"
                try:
                    _pool = psycopg2.pool.ThreadedConnectionPool(
                        minconn=1,
                    ...
                    _log.info('"Connected to PostgreSQL via socket (attempt %d)"', attempt)
                    return
                except psycopg2.Error as se:
                    e = se
