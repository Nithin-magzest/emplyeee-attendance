...
Missing-globals injection is replicated here so this file can run independently.
"""
import datetime
import io
...
# ── Autouse: inject app.py globals that attendance.py never imports ──────────
...
class TestGetKnownFaceEncoding:
    ...
    def test_cache_hit_returns_cached_encoding(self, mocker):
        ...
        sentinel = object()
        mtime = 12345.0
        att._face_enc_cache["CACHE_HIT_EMP"] = (mtime, sentinel)
        ...
        del att._face_enc_cache["CACHE_HIT_EMP"]
    ...
    def test_cache_miss_calls_face_recognition(self, mocker):
        ...
        mocker.patch.object(att, "face_recognition", fr_mock)
        att._face_enc_cache.pop("MISS_EMP", None)
        result = att._get_known_face_encoding("MISS_EMP", "/fake/path.jpg")
        assert result == fake_enc
        assert att._face_enc_cache.get("MISS_EMP") == (mtime, fake_enc)
        del att._face_enc_cache["MISS_EMP"]
    ...
    def test_no_encodings_returns_none_cached(self, mocker):
        ...
        mocker.patch.object(att, "face_recognition", fr_mock)
        att._face_enc_cache.pop("NOENC_EMP", None)
        result = att._get_known_face_encoding("NOENC_EMP", "/fake/path.jpg")
        assert result is None
        del att._face_enc_cache["NOENC_EMP"]
...
class TestSafeReferrerRedirect:
    ...
    def test_relative_url_passes_to_safe_redirect(self, client, mocker):
        import blueprints.attendance as att
        mocker.patch.object(att, "_safe_redirect", return_value="/settings")
        with client.application.test_request_context("/"):
            result = att._safe_referrer_redirect("/settings?tab=shifts", "/fallback")
            assert result == "/settings"
            att._safe_redirect.assert_called_once()
...
class TestMobileBiometricRecentlyVerified:
    ...
    def test_recent_proof_returns_true_and_clears(self, db_engine, seed_employee, client):
        ...
        import blueprints.attendance as att
        with client.application.test_request_context("/"):
            result = att._mobile_biometric_recently_verified(emp_id)
        assert result is True
    ...
    def test_empty_emp_id_returns_false(self, client):
        import blueprints.attendance as att
        with client.application.test_request_context("/"):
            result = att._mobile_biometric_recently_verified("")
        assert result is False
...
class TestKioskLoginStatus:
    ...
    def _login_fresh(self, client, seed_employee, db_engine, mocker, s_start, s_half, grace):
        ...
        mocker.patch.object(att, "GRACE_MINUTES", grace)
...
class TestApiCheckinStatusBranches:
    ...
    def _patch_shifts(self, att, s_start, s_half, s_end, grace=0):
        att.SHIFT_START = s_start
        att.SHIFT_HALF  = s_half
        att.SHIFT_END   = s_end
        att.GRACE_MINUTES = grace

    def _restore_shifts(self, att):
        att.SHIFT_START   = datetime.time(9, 0)
        att.SHIFT_HALF    = datetime.time(13, 0)
        att.SHIFT_END     = datetime.time(18, 0)
        att.GRACE_MINUTES = 15
...
class TestQrFaceCheckinCoverage:
    ...
    def test_optional_face_photo_saved_when_not_needed(self, client, seed_employee, db_engine, mocker):
        ...
        import blueprints.attendance as att
        mocker.patch.object(att, "GRACE_MINUTES", 0)
        mocker.patch.object(att, "SHIFT_START", datetime.time(0, 0))
        mocker.patch.object(att, "SHIFT_HALF", datetime.time(23, 59))
        mocker.patch.object(att, "SHIFT_END", datetime.time(23, 59))
        rv = client.post("/api/employee/qr-face-checkin", data={
            "employee_id": seed_employee["employee_id"],
    ...
    def test_qr_face_known_enc_none_returns_400(self, client, seed_employee, db_engine, mocker):
        ...
        mocker.patch("blueprints.attendance._face_recognition_available", True)
        mocker.patch("blueprints.attendance.os.path.exists", return_value=True)
        mocker.patch("blueprints.attendance._get_known_face_encoding", return_value=None)
    ...
    def test_half_day_login_qrface(self, client, seed_employee, db_engine, mocker):
        ...
        mocker.patch.object(att, "GRACE_MINUTES", 0)
        mocker.patch.object(att, "SHIFT_START", datetime.time(0, 0))
        mocker.patch.object(att, "SHIFT_HALF", datetime.time(0, 1))
        mocker.patch.object(att, "SHIFT_END", datetime.time(23, 59))
        rv = client.post("/api/employee/qr-face-checkin", data={
            "employee_id": seed_employee["employee_id"],
            "auth_combo":  "qr_fingerprint",
