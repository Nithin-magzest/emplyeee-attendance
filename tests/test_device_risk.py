...
already-computed by a trusted caller and, above 65, force-compromises the
...
class TestDeviceRiskEndpoint:
    ...
    def test_score_at_threshold_does_not_block(self, client, seed_employee):
        """> 65 blocks, per spec — exactly 65 should not."""
        with client.session_transaction() as sess:
            sess["employee_id"] = seed_employee["employee_id"]
        resp = client.post("/api/employee/device_risk", json={"risk_score": 65})
    ...
    def test_score_out_of_range_is_clamped_not_rejected(self, client, seed_employee):
        ...
        assert resp.status_code == 200
        assert resp.get_json()["blocked"] is True  # clamped to 100, still > 65
