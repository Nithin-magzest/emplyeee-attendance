...
export default function ManagerDrawerContent(props) {
  ...
  const activeRoute = state?.routes[state?.index]?.name || "ManagerDashboard";

  const menuItems = [
    { title: "Team Dashboard", icon: "briefcase-outline", iconFocused: "briefcase", route: "ManagerDashboard" },
    { title: "Team Attendance", icon: "calendar-outline", iconFocused: "calendar", route: "Attendance" },
    { title: "Mark Team Attendance", icon: "create-outline", iconFocused: "create", route: "MarkAttendance" },
    { title: "Team Approvals", icon: "checkmark-done-circle-outline", iconFocused: "checkmark-done-circle", route: "LeaveRequests", badge: "2 NEW" },
    { title: "Direct Reports", icon: "people-outline", iconFocused: "people", route: "Employees" },
    { title: "Comp-off Claims", icon: "time-outline", iconFocused: "time", route: "CompOff" },
    { title: "Team KPIs", icon: "trending-up-outline", iconFocused: "trending-up", route: "Performance" },
    { title: "Team Support Tickets", icon: "ticket-outline", iconFocused: "ticket", route: "Tickets" },
  ];
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        <View style={styles.headerRow}>
          <View style={styles.avatarBorder}>
            <View style={styles.avatar}>
              <Ionicons name="briefcase" size={24} color="#0B2253" />
            </View>
          ...
          <View style={styles.userInfo}>
            <Text style={styles.name} numberOfLines={1}>
              {user?.company || user?.name || "Engineering & Operations"}
            </Text>
            <Text style={styles.empId} numberOfLines={1}>
              {user?.name || "Team Lead Manager"}
            </Text>
            <View style={styles.roleBadgeRow}>
              <View style={styles.roleBadge}>
                <Ionicons name="people" size={12} color="#22C55E" style={{ marginRight: 4 }} />
                <Text style={styles.roleText}>Line Manager & Team Lead</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      <DrawerContentScrollView {...props} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>TEAM OPERATIONS & REPORTS</Text>
          {menuItems.map((item) => {
      ...
      <View style={[styles.bottomContainer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        ...
        <Text style={styles.version}>Magzest HRMS SaaS • Team Hub v1.0.0</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  pillBadge: { backgroundColor: "#DCFCE7", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, marginRight: 8 },
  pillBadgeText: { fontSize: 10, fontWeight: "800", color: "#16A34A" },
