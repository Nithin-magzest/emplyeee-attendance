...
export default function HrDrawerContent(props) {
  const { navigation, state } = props;
  const { user, signOut } = useAuth();
  const insets = useSafeAreaInsets();
  const activeRoute = state?.routes[state?.index]?.name || "HrDashboard";

  const menuItems = [
    { title: "HR Dashboard", icon: "people-outline", iconFocused: "people", route: "HrDashboard" },
    { title: "Employee Roster", icon: "card-outline", iconFocused: "card", route: "Employees" },
    { title: "Leave Approvals", icon: "calendar-outline", iconFocused: "calendar", route: "LeaveRequests", badge: "PENDING" },
    { title: "Onboarding Tasks", icon: "clipboard-outline", iconFocused: "clipboard", route: "Onboarding" },
    { title: "Performance Reviews", icon: "trending-up-outline", iconFocused: "trending-up", route: "Performance" },
    { title: "Leaves & Holidays", icon: "calendar-number-outline", iconFocused: "calendar-number", route: "LeavesHolidays" },
    { title: "HR Tickets", icon: "ticket-outline", iconFocused: "ticket", route: "Tickets" },
    { title: "Resignation Requests", icon: "exit-outline", iconFocused: "exit", route: "Resignations" },
    { title: "Departments & Units", icon: "business-outline", iconFocused: "business", route: "Departments" },
    { title: "Organization Hierarchy", icon: "git-network-outline", iconFocused: "git-network", route: "OrgChart" },
  ];
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        <View style={styles.headerRow}>
          <View style={styles.avatarBorder}>
            <View style={styles.avatar}>
              <Ionicons name="person" size={24} color="#0B2253" />
            </View>
          ...
          <View style={styles.userInfo}>
            <Text style={styles.name} numberOfLines={1}>
              {user?.company || user?.name || "People & Culture"}
            </Text>
            ...
            <View style={styles.roleBadgeRow}>
              <View style={styles.roleBadge}>
                ...
                <Text style={styles.roleText}>Human Resources Lead</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      <DrawerContentScrollView {...props} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>HR OPERATIONS & MANAGEMENT</Text>
          {menuItems.map((item) => {
      ...
      <View style={[styles.bottomContainer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        <TouchableOpacity activeOpacity={0.88} style={styles.logoutButton} onPress={signOut}>
          ...
          <Text style={styles.logoutText}>Sign Out HR Portal</Text>
        </TouchableOpacity>

        <Text style={styles.version}>Magzest HRMS SaaS • HR Portal v1.0.0</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  pillBadge: { backgroundColor: "#FEF3C7", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, marginRight: 8 },
  pillBadgeText: { fontSize: 10, fontWeight: "800", color: "#D97706" },
