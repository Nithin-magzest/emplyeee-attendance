...
def init_db():
    ...
    try:
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute("""
                INSERT INTO security_events (event_type, level, message, identifier, ip, path, method)
                VALUES 
                ('secops.system_init', 'INFO', 'Security Operations Center SIEM telemetry initialized', 'sp_admin', '127.0.0.1', '/secops', 'GET'),
                ('antivirus.daemon_active', 'INFO', 'Asynchronous filesystem scanner active — 0 malware threats in /uploads', 'system-daemon', '127.0.0.1', '/api/secops/quarantine', 'GET')
            """)
            db.commit()
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute("""
                ...
                VALUES
                ('suspicious_resume_payload.pdf.exe', 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855', 'EMP009', '/uploads/quarantine/inv_901.exe', 'Win32.Trojan.Downloader.Gen', 'Quarantined')
            """)
            db.commit()
    except Exception:
        db.rollback()
...
if "core.home" not in app.view_functions:
    ...
    for _bp in (health_bp, notifications_bp, payroll_bp, leave_bp, admin_views_bp,
                ...
                documents_bp, org_bp, onboarding_bp, employee_portal_bp, core_bp, ai_hrms_bp, secops_bp, compliance_bp, automation_bp):
